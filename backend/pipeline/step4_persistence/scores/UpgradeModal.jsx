...
// Compute fatigue mathematically based on the derivative of the time series
function computeFatigueDay(values) {
  if (!values || values.length < 2) return null;

  // Find the exact Peak CTR day to ignore initial warmups
...
export default function UpgradeModal({ creative, isOpen, onClose, onApply }) {
  ...
  // Build series for chart combining both Original and Upgraded predictions
  const buildComparisonSeries = (predObj) => {
    if (!predObj?.original?.predictions || !predObj?.generated?.predictions) return [];
    const all = [];

    // Map Original
    predObj.original.predictions.forEach((p, i) => {
      // Use the backend's provided fatigue day if it exists, otherwise calculate locally
      const fatigue_day = Math.floor(p.fatigue_day) || null;
          ...
          <div className="mb-6 md:mb-8 flex items-center justify-between">
            ...
            {upgradedData?.success && (
              <button
                ...
                className={`px-4 py-2 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${showForecast
                ...
                <div className="text-center w-full">
                  ...
                  <div className="space-y-2 text-left max-w-xs mx-auto">
                    {UPGRADE_STEPS.map((step, i) => (
                      <div key={i} className={`flex items-center gap-3 text-[11px] font-bold transition-all ${i < currentStep ? 'text-emerald-600' : i === currentStep ? 'text-indigo-600 animate-pulse' : 'text-slate-300'
                        }`}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {COUNTRIES.map(c => (
                      <button key={c} onClick={() => toggleCountry(c)}
                        className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all ${selectedCountries.includes(c)
                  ...
                  <div className="flex gap-1.5">
                    {OS_OPTS.map(o => (
                      <button key={o} onClick={() => toggleOS(o)}
                        className={`px-3 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wide transition-all ${selectedOS.includes(o)
                          ? 'bg-indigo-600 text-white'
                          : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                          }`}>{o}</button>
                    ))}
              ...
              {!predLoading && prediction && (
                <div className="flex-1 flex flex-col">
                  ...
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-2 text-center">
                    Predicted 30-day CTR comparison · {prediction.generated?.summary.best_segment} is top AI segment
                  </p>
                </div>
              )}
            </div>
